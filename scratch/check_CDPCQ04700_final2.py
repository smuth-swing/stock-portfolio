...
for k in res.keys():
    if k.endswith('OutBlock2') or k.endswith('OutBlock3'):
        print(f"{k} type: {type(res[k])}")
        if isinstance(res[k], list) and len(res[k]) > 0:
            print(json.dumps(res[k][0], indent=2, ensure_ascii=False)[:1000])
        elif isinstance(res[k], dict):
            print(json.dumps(res[k], indent=2, ensure_ascii=False)[:1000])