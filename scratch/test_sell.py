...
def test_portfolio_sync(file_path, stock_name, new_total_amount):
    print(f"--- Testing sync for {stock_name} with new total {new_total_amount} ---")
    ...
    if not target_row:
        print(f"Error: Stock {stock_name} not found")
        return
    ...
    print(f"Target ones: {target_ones}")
...
def create_mock_excel():
    ...
    # 3 existing ones
    for i in range(5, 8):
        ws.cell(row=2, column=i, value=1).fill = yellow_fill
    wb.save("test_portfolio_sell.xlsx")

if __name__ == "__main__":
    create_mock_excel()
    # Scenario: Current 300, sell 100 -> new total 200
    test_portfolio_sync("test_portfolio_sell.xlsx", "리가켐바이오", 200)
    
    wb = openpyxl.load_workbook("test_output_sell.xlsx")
    ws = wb['포트폴리오 맵']
    final_count = 0
    for c in range(5, 100):
